...
class FilterAnalysisScheme(Scheme, ABC):
    ...
    def __call__(self) -> None:
        ...
        while c.time < cf.time_end:
            ...
            ##assimilation step
            if cf.run_analysis and c.time >= cf.time_analysis_start and c.time <= cf.time_analysis_end:
                self.filter(cf)
                if cf.run_postproc:
                    self.postprocess(cf)
    ...
    @abstractmethod
    def preprocess(self, c: Config) -> None:
        """
    ...
    @abstractmethod
    def postprocess(self, c: Config) -> None:
        """
