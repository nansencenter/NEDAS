...
class Att_Res_UNet():
    ...
    def repeat_elem(self, tensor, rep):
        return keras.layers.Lambda(lambda x, repnum: keras.backend.repeat_elements(x, repnum, axis = 3), arguments = {'repnum': rep})(tensor)

    def gating_signal(self, x, n_filters, batch_norm = False):
        x = keras.layers.Conv2D(n_filters, (1,1), padding = "same")(x)
        if batch_norm == True:
    ...
    def attention_block(self, x, g, inter_shape):
        ...
        shape_g = keras.backend.int_shape(g)

        theta_x = keras.layers.Conv2D(inter_shape, kernel_size = (2,2), strides = (2,2), padding = "same")(x)
    ...
    def residual_conv_block(self, x, n_filters, padding: Literal["valid", "same"] = "same", kernel_size=(3,3)):
        ...
        conv = keras.layers.Conv2D(n_filters, kernel_size = kernel_size, padding = padding, kernel_initializer = self.kernel_initializer)(conv)
        if self.batch_norm == True:
            conv = keras.layers.BatchNormalization(axis = 3)(conv)

        shortcut = keras.layers.Conv2D(n_filters, kernel_size = (1,1), padding = padding)(x)
        if self.batch_norm == True:
            shortcut = keras.layers.BatchNormalization(axis = 3)(shortcut)

        res_path = keras.layers.add([shortcut, conv])
        res_path = keras.layers.Activation(self.activation)(res_path)
    ...
    def downsample_block(self, x, n_filters, pool_size = (2,2), kernel_size = (3,3), strides = 2):
        ...
        if self.pooling_type == "Max":
            p = keras.layers.MaxPool2D(pool_size = pool_size, strides = strides)(f)
        elif self.pooling_type == "Average":
            p = keras.layers.AveragePooling2D(pool_size = pool_size, strides = strides)(f)
        else:
    ...
    def upsample_block(self, x, conv_features, n_filters, kernel_size = (3,3), strides = 2, padding = "same"):
        ...
        up_att = keras.layers.UpSampling2D(size = (2, 2), data_format = "channels_last")(x)
        up_att = keras.layers.concatenate([up_att, att], axis = 3)
    ...
    def make_unet_model(self): 
        ...
        # outputs
        SICerror = keras.layers.Conv2D(self.n_targets, (1, 1), padding = "same", activation = "linear", dtype = float32, name = "psi")(u0)
        unet_model = keras.Model(inputs, SICerror, name = "U-Net")
