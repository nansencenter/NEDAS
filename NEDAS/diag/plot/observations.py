...
def get_task_list(c: Context, **kwargs) -> list:
    ...
    # single Obs construction — local variable, does not mutate c
    obs = Obs(c)
    ...
    for i, vname in enumerate(variables):
        obs_rec_query = [id for id, r in obs.info.records.items()
                         if r.name == vname and r.dataset_src == dataset_src[i]]
        assert len(obs_rec_query) > 0, \
            f"cannot find obs record for '{vname}' from dataset '{dataset_src[i]}'"
        obs_rec_id = obs_rec_query[0]
        obs_rec = obs.info.records[obs_rec_id].asdict()
        ...
        for k in levels:
            for t in obs_ts:
                for m in range(c.nens):
                    tasks.append({**kwargs,
                                  'obs_rec_id': obs_rec_id, 'obs_rec': obs_rec,
                                  'member': m, 'k': k, 't': t, 'dt': obs_dt[i],
                                  'vmin': vmin[i], 'vmax': vmax[i], 'nlevels': nlevels[i], 'cmap': cmap[i],
...
def run(c: Context, **kwargs) -> None:
    """
    Run diagnostics: plot the observation sequence and obs-prior departure
    """
    if 'plot_dir' in kwargs:
        plot_dir = kwargs['plot_dir']
    else:
    ...
    # local Obs instance — needed only for get_ref_z, does not mutate c
    obs = Obs(c)
    model_z = obs.get_ref_z(c, obs_rec['model_src'], c.time)
    if k == 0:
    ...
    try:
        ...
        if obs_rec['is_vector']:
            ...
            obs_val = np.array([obs_u, obs_v])
            c.grid.plot_scatter(ax[0], obs_val, vmin, vmax, nlevels, x=obs_x[ind], y=obs_y[ind], is_vector=True, units=obs_rec['units'])
            adjust_ax_size(ax[0])

            obs_prior_u = obs_prior_seq[0, ...][ind]
            obs_prior_v = obs_prior_seq[1, ...][ind]
            obs_prior = np.array([obs_prior_u, obs_prior_v])
            obs_diff = obs_val - obs_prior
            ...
            obs_val = obs_seq['obs'][ind]
            c.grid.plot_scatter(ax[0], obs_val, vmin, vmax, nlevels, cmap=cmap, markersize=10, x=obs_x[ind], y=obs_y[ind])  # type: ignore
...
def generate_viewer_html(c, plot_dir, figsize, **kwargs) -> None:
    """Generate a static HTML viewer page for browsing the observation plots."""
    ...
    for i, vname in enumerate(variables):
        ...
        times_str += f"'{name}': ["
        for t in obs_ts:
            times_str += f"'{t:%Y%m%dT%H%M%S}_{t+obs_dt[i]*dt1h:%Y%m%dT%H%M%S}', "
        times_str += "], \n"
