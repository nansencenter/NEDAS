...
outdir = cc.SCRATCH+'/nextsim_ens_runs/wind10m_err1.0/'

cmap = [plt.cm.rainbow(m) for m in np.linspace(0, 1, 91)]
t0 = datetime(2007, 1, 1, 6, 0, 0)
dt = timedelta(hours=6)

ke_spec1 = np.load(outdir+'/output/001/si_ke_spec.npy')
...
for i in range(nens):
    ke_spec[i, :, :] = np.load(outdir+'/output/{:03d}/si_ke_spec.npy'.format(i+1))[:, 0:nt]
    ke_pert_spec[i, :, :] = np.load(outdir+'/output/{:03d}/si_ke_pert_spec.npy'.format(i+1))[:, 0:nt]
...
for n in np.arange(nt):
    ...
    ax.set_title('ice drift KE (m2/s2) '+tstr, fontsize=14)
