...
class AMSR2SeaIceConcObs(Dataset):
    proj_name: str
    proj: str
    xstart: float
    xend: float
    ...
    def filename(self, **kwargs):
        ...
        search = ''
        if time is None:
            search = os.path.join(path, '????', '??', 'ice_conc_'+self.proj_name+'-100_amsr2_????????????.nc')
            ...
            for d in d_range:
                ...
                search = os.path.join(path, f'{t:%Y}', f'{t:%m}', 'ice_conc_'+self.proj_name+'-100_amsr2_'+tstr+'.nc')
