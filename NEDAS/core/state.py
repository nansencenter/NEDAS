    ...
    def collect_prior_fields(self, c: Coordinator) -> None:
        ...
        for m, mem_id in enumerate(self.mem_list[c.pid_mem]):
            for r, rec_id in enumerate(self.rec_list[c.pid_rec]):
                ...
                path = c.io.forecast_dir(rec.time, model_name)
                model_fld = model.read_var(path=path, member=mem_id, **rec.asdict())
        ...
        # if c.debug:
        #     np.savez(f"{c.work_dir}/tmp/fields_prior_{c.pid_mem}_{c.pid_rec}.npz", **self.fields_prior)
