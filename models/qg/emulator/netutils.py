...
class Data_generator(tf.keras.utils.Sequence):
    def __init__(self, nrun, startrun, shuffle, batch_size, dim,  path_data, nfeatures=2, npredictors=2, sampleperrun = 100):
        self.nrun = nrun
    ...
    def __data_generation(self, list_r_s):  # Generates data containing batch_size samples
        ...
        X = np.full((self.batch_size, *self.dim, self.nfeatures), np.nan)
        y = np.full((self.batch_size, *self.dim, self.npredictors), np.nan)
        ...
        for i, (r,s) in enumerate(list_r_s):
            ...
            X[i] = ncx.variables['psi'][0,:,:,:].transpose((1,2,0))
            y[i] = ncy.variables['psi'][0,:,:,:].transpose((1,2,0))
            ncx.close()
            ncy.close()
        return (X, y)
