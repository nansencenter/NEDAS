...
class TestObsIncrementEAKF(unittest.TestCase):
    ...
    def test_zero_spread_gives_zero_increment(self):
        ...
        incr = _plain_obs_increment(prior, obs=5.0, obs_err=1.0)
    ...
    def test_posterior_mean_is_weighted_average(self):
        ...
        obs, obs_err = 3.0, 1.0
        incr = _plain_obs_increment(prior, obs=obs, obs_err=obs_err)
        post = prior + incr
    ...
    def test_posterior_variance_reduced(self):
        ...
        incr = _plain_obs_increment(prior, obs=1.0, obs_err=2.0)
    ...
    def test_posterior_variance_formula(self):
        ...
        obs_err = 1.5
        incr = _plain_obs_increment(prior, obs=1.0, obs_err=obs_err)
        prior_var = np.var(prior, ddof=1)
        var_ratio = obs_err**2 / (prior_var + obs_err**2)
    ...
    def test_increment_mean_equals_mean_shift(self):
        ...
        incr = _plain_obs_increment(prior, obs=5.0, obs_err=0.5)
...
class TestUpdateEnsembleEAKF(unittest.TestCase):
    ...
    def test_no_update_when_obs_prior_var_zero(self):
        ...
        ens_post = _plain_update_ensemble(ens_prior.copy(), obs_prior, obs_incr, np.ones(3))
    ...
    def test_zero_localization_gives_no_update(self):
        ...
        ens_post = _plain_update_ensemble(ens_prior.copy(), obs_prior, obs_incr, np.zeros(5))
        np.testing.assert_array_equal(ens_post, ens_prior)
...
class TestEAKFHybrid(unittest.TestCase):
    ...
    def test_no_static_members_matches_plain_eakf(self):
        ...
        obs_incr_ref = _plain_obs_increment(obs_prior, self.obs, self.obs_err)
        ens_post_ref = _plain_update_ensemble(self.ens_dynamic.copy(), obs_prior, obs_incr_ref, np.ones(self.nstate))
        for hybrid_perturbation in (False, True):
            obs_incr = obs_increment_eakf(obs_prior, np.zeros(0), self.obs, self.obs_err, 1.0, 0.0,
                                                 hybrid_perturbation)
