...
##s_units: source units used in dataset or models (defined in var_dict in each module)
def units_convert(units, s_units, var):
    ##  list of available units and methods to convert from s_units
    unit_dict = {'m/s':     {'km/h':lambda x: x/3.6,
    ...
    if units != s_units:
        if s_units not in unit_dict[units]:
            raise ValueError("cannot find convert method from "+s_units+" to "+units)
        else:
            print("converting from "+s_units+" to "+units)
            var = unit_dict[units][s_units](var)
    return var
