...
## The entire state has dimensions: member, variable, time, and the 3D space z, y, x.
## To organize the parallel tasks, we consider 3 indices: member, record, and location
## mem_id (0, ..., nens-1) indexes the ensemble members
## point_list is a list of point_id that indexes the spatial locations in a 2D field
## field_list is a list of (mem_id, rec_id) that indexes the uniq 2D fields

##Note: only regular grid is supported for analysis now

## This function parses info for the nens x nfld fields in the ensemble state.
## returns: info dict with some dimensions and lists
## -  info['fields'] points to records with uniq variable name, time and z (indexed by k),
##    since nt and nz may vary for different variables, we stack them together in the rec dimension.
## -  info['fld_list_proc'][pid] is a list of (mem_id, rec_id) for each pid
## -  info['loc_list_proc'][pid] is a list of location slices (ist,ied,di,jst,jed,dj) for each pid
...
##routine to prepare the state variables, getting the fields from model restart files,
...
##        fld_list_proc: list of field records to work on for each pid, each item
##                       is (mem_id,rec_id) referring to a uniq field
##        loc_list_proc: list of spatial locations to work on for each pid, each item
##                       is (istart,iend,di,jstart,jend,dj) that describes a slice of the domain
##returns: state dict with complete (mem_id, rec_id) pointing to the location slices in
##         in loc_list_proc for each pid
def prepare_state(c, comm, state_info):
    ...
    for task in range(ntask_max):
        ...
        ##2. Perform communication to transpose the state from field-complete to ensemble-complete
        ## For each source proc id (src_pid) with fld_list item (mem_id, rec_id), send chunk of
        ## fld[..., jstart:jend:dj, istart:iend:di] to destination proc id (dst_pid) with its
        ## corresponding slice in loc_list_proc
        ## All pid needs to send/recv to/from all pid, so we use cyclic coreography here
        ...
        ##    because they are waiting to receive before being able to send stuff themselves;
        ##    when finished with dst_pid>=pid, cycle back to send to dst_pid<pid, i.e.,
        ##    dst_pid list is [pid, pid+1, ..., nproc-1, 0, 1, ..., pid-1]
        if task < len(fld_list_proc[pid]):
