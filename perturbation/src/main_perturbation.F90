program main_perturbation
    ...
    real(8), allocatable, dimension(:,:,:,:) :: synforc, synforc_prev
    ...
    if(i_sample>0) then
        ...
        if(stat>0) stop 'previous synforc file not found'
        do i=1,n_field
            stat = nf90_inq_varid(ncid_prev, field(i)%name, varid_prev(i))
        end do
        do i=1,n_field
            stat = nf90_get_var(ncid_prev, varid_prev(i), synforc_prev(1:xdim,1:ydim,1:nens,i))
        enddo
        stat =  nf90_close(ncid_prev)
    endif

    !!!generate and update random field
    call rand_update(synforc, i_sample, synforc_prev)

    !!!write the random field to synforc.nc
    ...
    deallocate(synforc, synforc_prev)
