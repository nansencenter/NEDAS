...
while c.time < c.time_end:
    ...
    # the config file is dumped at every time step
    ...
    # locating the script for the model to working directory
    # # os.system("rsync -auv /bettik/aydogdu-ext/DATA/nextsim/dg/ " + cycle_dir + "/.")
    # shutil.copy(os.path.join("/bettik/aydogdu-ext/NEDAS/models/nextsim/dg/run.sh"),
    #                          os.path.join(cycle_dir, '.')
    #             )
    # os.chdir(cycle_dir)
    # os.system("sh run.sh " + cycle_dir)
